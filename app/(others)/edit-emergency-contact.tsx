...
import { updateContact, getContactById } from "@/services/api/emergency-contacts";
...
const EditEmergencyContactScreen = () => {
	...
	useEffect(() => {
		const fetchContact = async () => {
			try {
				...
				setIsCommanContact(contact.isCommanContact);
			} catch (error) {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			console.log("Error updating emergency contact:", {
				error,
				message: error?.message,
				response: error?.response?.data,
			});
			Alert.alert("Error", error?.response?.data?.message || "Failed to update emergency contact");
		} finally {
			setIsLoading(false);
		}
	};
		...
		<SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]}>
			...
			<ScrollView style={[styles.container, { backgroundColor: theme.background }]}>
				...
				>
					...
					<Text style={[styles.headerText, { color: theme.text }]}>Edit Emergency Contact</Text>
					<Text style={[styles.headerSubtext, { color: theme.textSecondary }]}>Update contact information</Text>
				</LinearGradient>
